...
nbOccurrences (h:q) = let n = length q + 1
                          qf = filter (/= h) q
                      in (h,n - length qf):nbOccurrences qf
nbOccurrences [] = []
...
poids (Node g d) = poids g + poids d
poids (Leaf (_,n)) = n
...
insere a (h:q) | ph < pa = h:insere a q
               | ph == pa = a:h:q
               | otherwise = h:a:q
               where ph = poids h
                     pa = poids a
...
triHuff = foldr (\x -> \fxs -> insere x fxs) []
...
agrege (h1:h2:q) = agrege (insere (Node h1 h2) q)
agrege [h1] = h1
agrege [] = error "Liste vide"
...
strip (Node g d) = Node (strip g) (strip d)
strip (Leaf (s,_)) = Leaf s
...
codeOne' :: Huff Char -> Char -> [Bits]
codeOne' (Node g d) c = map (L:) (codeOne' g c) ++ map (R:) (codeOne' d c)
...
decode a1 (Leaf a) l = a:(decode a1 a1 l)
decode a1 (Node g d) (h:q) | h == R = decode a1 d q
                           | h == L = decode a1 g q
decode _ _ [] = []
...
-- reponse : Stocker poids dans noeud (cf. TP3)